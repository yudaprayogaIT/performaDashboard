...
interface ComparisonCardProps {
  ...
  icon?: string;
  className?: string;
  lastUpdateOmzet?: string;
  lastUpdateGrossMargin?: string;
}
  ...
  return (
    <div className="pt-3 border-t border-white/10">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {isPositive && "+"}
          {formatPercentage(data.percentageChange, 1)}
        </span>
      </div>
    </div>
  );
...
export default function ComparisonCard({
  ...
  return (
    ...
    >
      ...
      {lastUpdateOmzet && (
        <p className="text-xs text-white/30 mb-3 pt-3 border-white/10 flex items-center gap-1">
          {/* <span className="material-symbols-outlined text-xs">schedule</span> */}
          Update: {lastUpdateOmzet}
        </p>
      )}
      ...
      {/* Last update timestamp */}
      {lastUpdateGrossMargin && (
        <p className="text-xs text-white/30 pt-3  border-white/10 flex items-center gap-1">
          {/* <span className="material-symbols-outlined text-xs">schedule</span> */}
          Update: {lastUpdateGrossMargin}
        </p>
      )}
    </div>
  );
